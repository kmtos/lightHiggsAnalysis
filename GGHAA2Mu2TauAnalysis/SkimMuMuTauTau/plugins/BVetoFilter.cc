...
class BVetoFilter : public edm::EDFilter {
  ...
  edm::EDGetTokenT<std::vector<reco::PFJet> > oldJetTag_;
...
BVetoFilter::BVetoFilter(const edm::ParameterSet& iConfig) :
  ...
  oldJetTag_ ( consumes<std::vector<reco::PFJet> >(iConfig.getParameter<edm::InputTag>("oldJetTag"))),
  jetMuonMapTag_(consumes<edm::ValueMap<reco::MuonRefVector>>(iConfig.getParameter<edm::InputTag>("jetMuonMapTag"))),
  bTagInfoTag_( consumes<edm::AssociationVector<edm::RefToBaseProd<reco::Jet>,std::vector<float>,edm::RefToBase<reco::Jet>,unsigned int,edm::helper::AssociationIdenticalKeyReference> >(iConfig.getParameter<edm::InputTag>("bTagInfoTag"))),
...
BVetoFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  iEvent.getByToken(bTagInfoTag_, pBTagInfo);
