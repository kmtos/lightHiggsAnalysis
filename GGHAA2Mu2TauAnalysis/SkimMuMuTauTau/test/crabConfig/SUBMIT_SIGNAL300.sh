...
oldMass=21
while [ $mass -lt 22 ]; do
  oldMassString1="h300a$oldMass"
  massString1="h300a$mass"
  oldMassString2="M-${oldMass}_"
  massString2="M-${mass}_"
  sed -i "s|${oldMassString1}|${massString1}|g" crabConfig_H300.py
  sed -i "s|${oldMassString2}|${massString2}|g" crabConfig_H300.py
  echo "Submitting $massString1 $massString2"
  crab submit crabConfig_H300.py >> $FILENAME
  let oldMass=mass
  let mass=mass+2
  echo "" >> $FILENAME
done
