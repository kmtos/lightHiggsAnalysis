...
void Common::sortByPT(std::vector<pat::Muon*>& objects)
{
  std::sort(objects.begin(), objects.end(), comparePATMuonPT);
}
...
bool Common::comparePATMuonPT(pat::Muon* object1, pat::Muon* object2)
{
  return (object1->pt() < object2->pt());
}
...
float Common::getMuonCombPFIsoModified(const reco::Muon& muon, const reco::PFTauRef& tau, 
				       const double PUSubtractionCoeff)
{
  const reco::MuonPFIsolation isoBlock = muon.pfIsolationR04();
  const std::vector<reco::PFCandidatePtr>& tauChargedCands = (*tau).signalPFChargedHadrCands();
  const std::vector<reco::PFCandidatePtr>& tauNeutralCands = (*tau).signalPFNeutrHadrCands();
  const std::vector<reco::PFCandidatePtr>& tauGammaCands = (*tau).signalPFGammaCands();

  double tauChargedHadronPt = 0.;
  double tauNeutralHadronPt = 0.;
  double tauPhotonPt = 0.;

  for (std::vector<reco::PFCandidatePtr>::const_iterator iCand = tauChargedCands.begin(); 
       iCand != tauChargedCands.end(); ++iCand)
    {
      if (deltaR(muon.p4(), (*iCand)->p4()) < 0.4)
	tauChargedHadronPt += (*iCand)->pt();
    }

  for (std::vector<reco::PFCandidatePtr>::const_iterator iCand = tauNeutralCands.begin(); 
       iCand != tauNeutralCands.end(); ++iCand)
    {
      if (deltaR(muon.p4(), (*iCand)->p4()) < 0.4)
	tauNeutralHadronPt += (*iCand)->pt();
    }

  for (std::vector<reco::PFCandidatePtr>::const_iterator iCand = tauGammaCands.begin(); 
       iCand != tauGammaCands.end(); ++iCand)
    {
      if (deltaR(muon.p4(), (*iCand)->p4()) < 0.4)
...
std::vector<reco::MuonRef>
Common::getTrackerRecoMuons(const edm::Handle<reco::MuonCollection>& pMuons)
{
  std::vector<reco::MuonRef> trackerMuons;
  for (reco::MuonCollection::const_iterator iMuon =pMuons->begin(); iMuon!= pMuons->end();
    ++iMuon){
  if(iMuon->isGlobalMuon())
    trackerMuons.push_back(reco::MuonRef(pMuons, iMuon -pMuons->begin()));
  }
  return trackerMuons;
}
std::vector<reco::MuonRef>
...
std::vector<reco::MuonRef>
Common::getTrackerRecoMuons(const edm::Handle<reco::MuonRefVector>& pMuons,
                            const edm::Handle<reco::MuonCollection>& pBaseMuons)
{
   std::vector<reco::MuonRef> trackerMuons;
   for (reco::MuonRefVector::const_iterator iMuon= pMuons->begin(); iMuon!= pMuons->end();
