...
// Class:      CustomTauSelector_IgnoreIso
// 
/**\class CustomTauSelector_IgnoreIso CustomTauSelector_IgnoreIso.cc 
   BoostedTauAnalysis/GenMatchedRecoObjectProducer/src/CustomTauSelector_IgnoreIso.cc
...
// $Id: CustomTauSelector_IgnoreIso.cc,v 1.6 2012/12/12 16:02:05 yohay Exp $
//
//
...
using namespace edm;
template<class T>
class CustomTauSelector_IgnoreIso : public edm::EDFilter {
public:
  explicit CustomTauSelector_IgnoreIso(const edm::ParameterSet&);
  ~CustomTauSelector_IgnoreIso();
...
private:
  ...
  std::vector<PFTauDiscriminatorToken> tauDMTag_;
...
//
template<class T>
CustomTauSelector_IgnoreIso<T>::CustomTauSelector_IgnoreIso(const edm::ParameterSet& iConfig) :
...
//  histos1D_()
{
  ...
  tauDMTag_.push_back(consumes<reco::PFTauDiscriminator>(iConfig.getParameter<edm::InputTag>("tauDMTag")));
...
template<class T>
CustomTauSelector_IgnoreIso<T>::~CustomTauSelector_IgnoreIso()
{
...
template<class T>
bool CustomTauSelector_IgnoreIso<T>::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //get tau DM
  std::vector<edm::Handle<reco::PFTauDiscriminator> > pTauDM(tauDMTag_.size(), edm::Handle<reco::PFTauDiscriminator>());
  for (std::vector<edm::EDGetTokenT<reco::PFTauDiscriminator> >::const_iterator iTag = tauDMTag_.begin();  iTag != tauDMTag_.end(); ++iTag)
    iEvent.getByToken(*iTag, pTauDM[iTag - tauDMTag_.begin()]);
  ...
  //fill STL container with taus passing specified discriminators in specified eta and pT range
   std::vector<reco::PFTauRef> tausPassDM = pTaus.isValid() ?
    Common::getRecoTaus(pTaus, pBaseTaus, pTauDM, pTauHadIso, pTMin_, etaMax_, true, isoMax_) :
    Common::getRecoTaus(pBaseTaus, pTauDM, pTauHadIso, pTMin_, etaMax_, true, isoMax_);
  ...
  for (std::vector<reco::PFTauRef>::const_iterator iTau = tausPassDM.begin(); iTau != tausPassDM.end(); ++iTau) 
  {
    if (fabs((*iTau)->charge() ) == 1)
    {
      ...
      bool checkDM = false;
      for (std::vector<reco::PFTauRef>::const_iterator iTauDM = tausPassDM.begin(); iTauDM != tausPassDM.end(); ++iTauDM)  
      {
        if (iTau->key() == iTauDM->key() )
          checkDM = true;
      }//for

      if ((!(pOverlapCands.isValid()) || ((nearestMuon != NULL) && (reco::deltaR(**iTau, *nearestMuon) > dR_)))&&(!(pOverlapCands1.isValid()) || ((nearestMuon1 != NULL) && (reco::deltaR(**iTau, *nearestMuon1) > dR_))) && checkDM) 
      {
...
template<class T>
void CustomTauSelector_IgnoreIso<T>::beginJob()
{
  if(debug_)
  {
...
template<class T>
void CustomTauSelector_IgnoreIso<T>::endJob() {
  if(debug_)
  {
    out_->cd();
...
template<class T>
bool CustomTauSelector_IgnoreIso<T>::beginRun(edm::Run&, edm::EventSetup const&)
{ 
  return true;
}
...
template<class T>
bool CustomTauSelector_IgnoreIso<T>::endRun(edm::Run&, edm::EventSetup const&)
{
  return true;
}
...
template<class T>
bool CustomTauSelector_IgnoreIso<T>::beginLuminosityBlock(edm::LuminosityBlock&, edm::EventSetup const&)
{
  return true;
}
...
template<class T>
bool CustomTauSelector_IgnoreIso<T>::endLuminosityBlock(edm::LuminosityBlock&, edm::EventSetup const&)
{
  return true;
}
...
template<class T>
void CustomTauSelector_IgnoreIso<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
typedef CustomTauSelector_IgnoreIso<reco::Muon> CustomTauSepFromMuonSelectorDMOnly;
typedef CustomTauSelector_IgnoreIso<reco::Photon> CustomTauSepFromPhotonSelectorDMOnly;
DEFINE_FWK_MODULE(CustomTauSepFromMuonSelectorDMOnly);
DEFINE_FWK_MODULE(CustomTauSepFromPhotonSelectorDMOnly);
